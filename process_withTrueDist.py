...
from utils import calculate_JSD, calculate_JSD_true, calculate_multivariate_Wasserstein, create_random_data_splt
...
def main(random_seeds, overwrite_string = []):
    ...
    # true_samples = aniso_GMM.sample(3000)
    # plt.scatter(true_samples[0][:, 0], true_samples[0][:, 1])
    
    # plt.scatter(fitting_dict['aniso_n_samples_6000_rnd_seed_0'][:, 0], fitting_dict['aniso_n_samples_6000_rnd_seed_0'][:, 1])
    # plt.show()
    ...
    for key, _ in fitting_dict.items():
        for config in testConfigs:
            ...
            true_pf[pf_key] = aniso_GMM if 'aniso' in key else varied_GMM if 'varied' in key else None
    ...
    for key, _ in fitting_pf.items():
        if key == 'varied_n_samples_6000_rnd_seed_0_config_cluster_PCA_stdKDE':
            print('Here')
